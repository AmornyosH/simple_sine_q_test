...
# ================================== Others ==================================
from GPDQ_NZM_matern import my_exact_gp
...
class GaussianProcessDiffusionQlearning:
    ...
    def loadTrainingRecord(self, path:str=None):
        ...
        self.gp_model.K = self.gp_model.maternKernel(self.gp_model.x_train, self.gp_model.x_train, noise=True)
    ...
    def predictGP(self, x_test):
        with torch.no_grad():
            ...
            _k_s = self.gp_model.maternKernel(X_1=self.gp_model.x_train, X_2=x_test, noise=False)
            _k_ss = self.gp_model.maternKernel(X_1=x_test, X_2=x_test, noise=False)
    ...
    def myGPTraining(self, total_epoch: int, ft: bool = False):
        ...
        _gradient_step = self.gp_model.num_sample // self.gp_model.gp_training_size
        _optimizer = torch.optim.Adam(self.gp_model.parameters(), lr=1e-02)
        self.gp_model.train()  # Set to training mode.

        _training_record = 0
        while _training_record < total_epoch:
            _start_time = time()
            for g in range(_gradient_step):
                _batch_x = self.gp_model.x_train_full[g * _batch_size:_batch_size + (g * _batch_size)]
                _batch_y = self.gp_model.y_train_full[g * _batch_size:_batch_size + (g * _batch_size)]
        ...
        self.gp_model.x_train = self.gp_model.x_train_full[0:self.gp_model.gp_training_size]
        self.gp_model.y_train = self.gp_model.y_train_full[0:self.gp_model.gp_training_size]
        ...
        # self.gp_model.eval()  # Set to evaluation mode after the training is finished.
