...
class myExactGP(torch.nn.Module):
    def __init__(self, params_dict:dict, dataset:dict, parent_alg:str, cuda:bool=False):
        ...
        if _ans_1 == 'n' or _ans_1=='N' or _ans_1=='No' or _ans_1=='NO':
            ...
            self.g_net = my_NN.MLP(input_dim=self.x_dim, output_dim=1, act_fn='relu')
        ...
        self.g_train = self.g_net(self.x_train)
        self.K = self.maternKernel(X_1=self.g_train, X_2=self.g_train, noise=True)
    ...
    def predict(self, X_s):
        with torch.no_grad():
            ...
            _k_s = self.maternKernel(X_1=self.g_train, X_2=g_test, noise=False)
            _k_ss = self.maternKernel(X_1=g_test, X_2=g_test, noise=False)
    ...
    def myTraining(self, total_epoch:int, ft:bool=False):
        ...
        _g_net_optimizer = torch.optim.Adam(self.g_net.parameters(), lr=1e-03)
