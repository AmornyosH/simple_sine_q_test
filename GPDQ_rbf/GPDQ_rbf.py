...
# ================================== Others ==================================
from GPDQ_rbf import my_exact_gp
...
class GaussianProcessDiffusionQlearning:
    def __init__(self, params_dict:dict, dataset:dict, ft:bool=False):
        ...
        self.checkpoint_path = '{:s}/training_records/{:s}_{:s}_checkpoint'.format(self.ALG, self.ALG, self.ENV_CONFIG)
    ...
    def loadTrainingRecord(self, path:str=None):
        ...
        # self.gp_model.K = self.gp_model.rbfKernel(self.gp_model.x_train, self.gp_model.x_train, noise=True)
    ...
    def training(self, total_epoch:int, eval:bool=False, ft:bool=False):
        ...
        while self.beh_training_record < total_epoch:
            ...
            self.recordSaving(path=self.checkpoint_path)
        ...
        while self.training_record < total_epoch:
            ...
            for g in range(_num_gradient_step):
                ...
                #     _gp_sampling_indices = torch.randperm(self.gp_model.gp_training_size)
                #     # Modified 
                #     # Use next action to save some time. 
                #     _batch_a_hat = self.gp_model.y_train[_gp_sampling_indices[0:_batch_size]]
                #     # _batch_a_hat = self.predict(state=self.gp_model.x_train, size=_batch_size, guide=True, dec_step=False).view(-1, self.ACTION_DIM)
                #     _forward_batch_a_hat_tensor = self.forwardProcess(data=_batch_a_hat, epsilon=epsilon_tensor, step=rand_t)
                #     _add_diffu_loss, _ = _trainDiffusionBeh(inputs=torch.concat([self.gp_model.x_train[_gp_sampling_indices[0:_batch_size]], _forward_batch_a_hat_tensor, encode_t_tensor], dim=1), y_true=epsilon_tensor)
                #     diffu_loss += _add_diffu_loss
            ...
            # Train the gp
            _gp_loss = self.gp_model.myTraining(total_epoch=1, ft=False)
