...
class myExactGP(torch.nn.Module):
    def __init__(self, params_dict:dict, dataset:dict, parent_alg:str, cuda:bool=False):
        ...
        if _ans_1 == 'n' or _ans_1=='N' or _ans_1=='No' or _ans_1=='NO':
            ...
            self.sigma_p = torch.nn.Parameter(_state_dict['sigma_p'], requires_grad=True)
            # self.mean = torch.nn.Parameter(_state_dict['mean'], requires_grad=True)

            self.mean = _training_records['mean']
