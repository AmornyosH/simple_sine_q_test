...
class MyCustomDQL:
    def __init__(self, params_dict:dict, dataset:dict, ft:bool=False):
        ...
        # Initialise Paths
        self.training_record_path = 'DQL/training_records/{:s}_{:s}_training_record'.format(self.ALG, self.ENV_CONFIG)
        self.training_record_2m_path = 'DQL/training_records/{:s}_{:s}_training_record_2m'.format(self.ALG, self.ENV_CONFIG)
        self.training_checkpoint_path = 'DQL/training_records/{:s}_{:s}_checkpoint'.format(self.ALG, self.ENV_CONFIG)
        self.evaluation_path = 'DQL/evaluation_data'
        ...
        if _ans_1 == 'n' or _ans_1=='N' or _ans_1=='No' or _ans_1=='NO':
            ...
            self.epsilon_tar = self.epsilon_beh
    ...
    def reverseProcess(self, inputs:list, size:int, target:bool=False):
    ...
    def predict(self, state, size:int, guide:bool=True, target:bool=False):
        ...
        return self.reverseProcess(state, size, target)
    ...
    def training(self, total_epoch:int, eval:bool=False, ft:bool=False):
        ...
        def _trainQ1Network(inputs, y_true):
            ...
            _y_pred = self.q_1(inputs)
            _q_1_loss = torch.mean(torch.square(y_true - _y_pred))
        ...
        def _trainQ2Network(inputs, y_true):
            ...
            _y_pred = self.q_2(inputs)
            _q_2_loss = torch.mean(torch.square(y_true - _y_pred))
        ...
        def _trainDiffusionBeh(inputs, y_true):
            ...
            # Q term
            _pred_a = self.predict(state=batch_state_tensor, size=_batch_size, target=True)
            if np.random.random() < 0.5:
                _q_pred = self.q_1(torch.concat([batch_state_tensor, _pred_a], dim=1))
                _q_norm = self.q_1(torch.concat([batch_state_tensor, batch_action_tensor], dim=1))
            else:
                _q_pred = self.q_2(torch.concat([batch_state_tensor, _pred_a], dim=1))
                _q_norm = self.q_2(torch.concat([batch_state_tensor, batch_action_tensor], dim=1))
            _diffu_loss -= (_ETA * torch.mean(_q_pred) / torch.mean(torch.abs(_q_norm)))
        ...
        def _updateTargetNetworks():
            ...
            epsilon_state_dict = self.epsilon_beh.state_dict()
            for key in epsilon_state_dict:
                epsilon_tar_state_dict[key] = epsilon_state_dict[key]*_TAU + epsilon_tar_state_dict[key]*(1-_TAU)
        ...
        while _training_record < total_epoch:
            ...
            for g in range(_num_gradient_step):
                ...
                _a_t_p_1 = self.predict(state=batch_next_state_tensor, size=_batch_size, target=True)
                y_true = _getExpectedCumulativeReturn(inputs=torch.concat([batch_next_state_tensor, _a_t_p_1], dim=1))
                q_1_loss = _trainQ1Network(inputs=torch.concat([batch_state_tensor, batch_action_tensor], dim=1), y_true=y_true) # State-Action network (Q)
                q_2_loss = _trainQ2Network(inputs=torch.concat([batch_state_tensor, batch_action_tensor], dim=1), y_true=y_true)  # State-Value network (V)
