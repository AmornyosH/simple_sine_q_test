...
class GaussianProcessDiffusionQlearning:
    ...
    def reverseProcess(self, inputs:list, size:int, guide:bool=False, dec_step:bool=False):
        ...
        for i in range(_diffu_steps): 
            ...
            # (Have to be inverted since the diffusion schedule was created in forward process's order.)
            ...
            if guide:
                ...
                _cov_gp = self.var_r * torch.eye(size, dtype=torch.float32)
    ...
    def getAlteredObservation(self, states):
        ...
        with torch.no_grad():
            ...
            for m in range(len(states)):
                ...
                _sampling_size = self.num_action_candidate
